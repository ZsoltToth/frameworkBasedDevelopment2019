...
then
	...
	docker exec -i $DOCKER_CONTAINER_NAME mysql -uroot -p$DB_ROOT_PASSWORD < ./sakila-db/sakila-schema.sql
	docker exec -i $DOCKER_CONTAINER_NAME mysql -uroot -p$DB_ROOT_PASSWORD < ./sakila-db/sakila-data.sql
fi
